...
class SuccessfulCreate(TestCase):
    ...
    def setUp(self):
        self.user = Client()
        temp = User.objects.create(email_address="test@uwm.edu", password="pass", first_name="Test", last_name="Test",
...
class MissingInformation(TestCase):
    ...
    def setUp(self):
        self.user = Client()
        temp = User.objects.create(email_address="testadmin@uwm.edu", password="pass", first_name="Test",
                                   last_name="Test", home_address="Random location", phone_number=9990009999)
        self.account = Administrator.objects.create(user=temp)
        ses = self.user.session
        ses["user"] = "testadmin@uwm.edu"
        ses.save()
