...
class SuccessfulCreation(TestCase):
    ...
    def setUp(self):
        ...
        self.course = Course.objects.create(course_id=100, semester="fall 2023", name="testCourse", description="test", num_of_sections=3, modality="online", credits=3)
        self.course.save()
    ...
    def test_creation(self):
        ...
        response = self.user.post("/home/managecourse/addinstructor/courseselect", {"selection", self.course}, follow=True)
        self.assertEquals(self.instructorToCourse, User.objects.get(self.instructorToCourse), "Did not successfully create account")
        self.assertRedirects(response, '/home/managecourse/addinstructor/courseselect/success')
...
class NoInstructor(TestCase):
    ...
    def test_no_instructor(self):
        ...
        self.assertEquals(resp.context["message"], "No Existing Instructors to Assign", "Cannot assign instructor when none exist")
...
class SuccessfulTransfer(TestCase):
    ...
    def test_instructor_to_next(self):
        resp = self.user.post("/home/managecourse/addinstructor", {"selection", self.instructor}, follow=True)
        self.assertEquals(resp.context["instructor"], self.instructor, "Instructor not transerred over properly")
