...
class SuccessfulLogin(TestCase):
    ...
    def test_login_with_valid_credentials(self):
        response = self.client.post('/login/', {'email': 'user@example.com', 'password': 'password'})
...
class InvalidLoginIncorrectPassword(TestCase):
    ...
    def test_login_with_incorrect_password(self):
        response = self.client.post('/login/', {'email': 'user@example.com', 'password': 'wrongpassword'})
...
class InvalidLoginNonexistentUser(TestCase):
    ...
    def test_login_with_nonexistent_user(self):
        response = self.client.post('/login/', {'email': 'nonexistent@example.com', 'password': 'password'})
        self.assertFormError(response, 'form', None, 'Invalid username or password')  # Assuming this error is shown
